...
from .model import MeasuredTuneResponseItem, MeasuredTuneResponsePerPowerConverter, MeasuredTuneResponse, TuneResponse, \
    RandomVariableMomenta, TuneResponseCollection
    ...
    for dev_name, val, tune_x, tune_y in zip_longest(
        ...
        tmp[str(dev_name)].append(
            MeasuredTuneResponseItem(
                value=float(val), x=float(tune_x), y=float(tune_y)
            )
        )

    # Now put it in a model
    r = MeasuredTuneResponse(col=[
            MeasuredTuneResponsePerPowerConverter(pc_name=dev_name, col=items) for dev_name, items in tmp.items()
    ])
    return r
